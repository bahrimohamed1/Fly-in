...
class PathFinder:
    ...
    def find_path(self, start_zone: Zone, end_zone: Zone, max_turns: int) -> Optional[List[PathStep]]:
        ...
        while queue:
            ...
            if next_turn > max_turns:
                continue
            if self.reservation_table.is_wait_valid(current_zone, current_turn):
            ...
            # check neibors
            for neighbor_zone, connection in self.graph.get_neighbors(current_zone.name):
                # blocked
                ...
                if neighbor_zone.zone_type == 'restricted':
                    ...
                    if next_turn > max_turns or arrival_turn > max_turns:
                        continue
                    if self.reservation_table.is_restricted_move_valid(current_zone, neighbor_zone, current_turn):
                ...
                # normal
                    ...
                    if next_turn > max_turns:
                        continue
                    if self.reservation_table.is_normal_move_valid(current_zone, neighbor_zone, current_turn):
                        new_step: PathStep = PathStep(
