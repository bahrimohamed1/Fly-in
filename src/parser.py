import re
import sys
from .zone import Zone, ZoneType
from .connection import Connection

VALID_ZONE_TYPES: set[str] = {zt.value for zt in ZoneType}


class Parser:
    def __init__(self, file_name: str) -> None:
        self.file_name: str = file_name

    def parse(self) -> tuple[list[Zone], list[Connection], int, str, str]:
        """Parse the map file.

        Returns:
            A tuple of (zones, connections, nb_drones, start_zone, end_zone).
        """
        zones: list[Zone] = []
        connections: list[Connection] = []
        nb_drones: int = 0
        start_zone: str = ""
        end_zone: str = ""
        has_nb_drones: bool = False
        defined_zones: set[str] = set()

        try:
            with open(self.file_name, "r") as file:
                raw_lines = file.readlines()
        except FileNotFoundError:
            print(f"Error: file not found: {self.file_name}")
            sys.exit(1)

        for line_num, raw_line in enumerate(raw_lines, 1):
            line = raw_line.strip()
            if not line or line.startswith('#'):
                continue

            meta_match = re.search(r"\[([^\]]*)\]", line)
            meta_str = (
                "[" + meta_match.group(1) + "]" if meta_match else ""
            )
            line_clean = re.sub(r"\s*\[.*?\]", "", line).strip()

            if not has_nb_drones:
                if not line_clean.startswith("nb_drones:"):
                    print(
                        f"Parse error at line {line_num}: "
                        "first non-comment line must be 'nb_drones: <N>'"
                    )
                    sys.exit(1)
                try:
                    nb_drones = int(line_clean.split(':', 1)[1].strip())
                except ValueError:
                    print(
                        f"Parse error at line {line_num}: "
                        "nb_drones must be a positive integer"
                    )
                    sys.exit(1)
                if nb_drones <= 0:
                    print(
                        f"Parse error at line {line_num}: "
                        "nb_drones must be a positive integer"
                    )
                    sys.exit(1)
                has_nb_drones = True
                continue

            meta = self._parse_metadata(meta_str, line_num)

            if line_clean.startswith("start_hub:"):
                parts = line_clean.split(":", 1)[1].strip().split()
                zone = self._parse_zone(parts, meta, line_num)
                zones.append(zone)
                defined_zones.add(zone.name)
                start_zone = zone.name

            elif line_clean.startswith("end_hub:"):
                parts = line_clean.split(":", 1)[1].strip().split()
                zone = self._parse_zone(parts, meta, line_num)
                zones.append(zone)
                defined_zones.add(zone.name)
                end_zone = zone.name

            elif line_clean.startswith("hub:"):
                parts = line_clean.split(":", 1)[1].strip().split()
                zone = self._parse_zone(parts, meta, line_num)
                zones.append(zone)
                defined_zones.add(zone.name)

            elif line_clean.startswith("connection:"):
                conn_str = line_clean.split(":", 1)[1].strip()
                zone_a, zone_b = self._find_connection_zones(
                    conn_str, defined_zones, line_num
                )
                raw_cap = meta.get("max_link_capacity", "1")
                try:
                    max_cap = int(raw_cap)
                except ValueError:
                    print(
                        f"Parse error at line {line_num}: "
                        "max_link_capacity must be a positive integer"
                    )
                    sys.exit(1)
                if max_cap <= 0:
                    print(
                        f"Parse error at line {line_num}: "
                        "max_link_capacity must be a positive integer"
                    )
                    sys.exit(1)
                connections.append(
                    Connection(
                        zone_a=zone_a,
                        zone_b=zone_b,
                        max_link_capacity=max_cap,
                    )
                )

        return zones, connections, nb_drones, start_zone, end_zone

    def _parse_metadata(
        self, meta_str: str, line_num: int
    ) -> dict[str, str]:
        """Parse [key=value ...] metadata string into a dict."""
        meta: dict[str, str] = {}
        if not meta_str:
            return meta
        inner = meta_str.strip()[1:-1]
        for pair in inner.split():
            if "=" not in pair:
                print(
                    f"Parse error at line {line_num}: "
                    f"invalid metadata token '{pair}'"
                )
                sys.exit(1)
            k, v = pair.split("=", 1)
            meta[k.strip()] = v.strip()
        return meta

    def _parse_zone(
        self,
        parts: list[str],
        meta: dict[str, str],
        line_num: int,
    ) -> Zone:
        """Parse zone attributes from token list and metadata dict."""
        if len(parts) < 3:
            print(
                f"Parse error at line {line_num}: "
                "zone definition requires: name x y"
            )
            sys.exit(1)
        name = parts[0]
        try:
            x = int(parts[1])
            y = int(parts[2])
        except ValueError:
            print(
                f"Parse error at line {line_num}: "
                "x and y coordinates must be integers"
            )
            sys.exit(1)

        zone_type_str = meta.get("zone", "normal")
        if zone_type_str not in VALID_ZONE_TYPES:
            print(
                f"Parse error at line {line_num}: "
                f"unknown zone type '{zone_type_str}'"
            )
            sys.exit(1)
        zone_type = ZoneType(zone_type_str)

        color = meta.get("color", "none")

        raw_md = meta.get("max_drones", "1")
        try:
            max_drones = int(raw_md)
        except ValueError:
            print(
                f"Parse error at line {line_num}: "
                "max_drones must be a positive integer"
            )
            sys.exit(1)
        if max_drones <= 0:
            print(
                f"Parse error at line {line_num}: "
                "max_drones must be a positive integer"
            )
            sys.exit(1)

        return Zone(
            name=name,
            x=x,
            y=y,
            zone_type=zone_type,
            color=color,
            max_drones=max_drones,
        )

    def _find_connection_zones(
        self,
        conn_str: str,
        defined_zones: set[str],
        line_num: int,
    ) -> tuple[str, str]:
        """Split 'zone_a-zone_b' into two known zone names."""
        indices = [i for i, ch in enumerate(conn_str) if ch == "-"]
        for idx in indices:
            candidate_a = conn_str[:idx]
            candidate_b = conn_str[idx + 1:]
            if candidate_a in defined_zones and candidate_b in defined_zones:
                return candidate_a, candidate_b
        print(
            f"Parse error at line {line_num}: "
            f"cannot resolve zones in connection '{conn_str}'"
        )
        sys.exit(1)
