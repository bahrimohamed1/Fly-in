...
class Parser:
    ...
    def _parse_zone_line(
        ...
        try:
            ...
            self._error(line_num, "x and y coordinates must be integers")
            raise  # satisfy mypy
    ...
    def parse(self) -> tuple[Graph, int]:
        ...
        Validates all directives and exits with a clear message on any
        error.
        """
        try:
        ...
        for line_num, raw_line in enumerate(raw_lines, 1):
            ...
            if line_clean.startswith("start_hub:"):
                ...
                if zone.name in defined_zones:
                    self._error(
                        line_num,
                        f"zone '{zone.name}' already defined",
                    )
                graph.add_zone(zone)
